...
router.post("/", async (req, res) => {
    try {
        ...
        // Ask Gemini to detect if this is a new product query or a follow-up
        const result = await model.generateContent(`
            You are a professional and friendly shopping assistant.

            Step 1: If the user's input is ONLY a greeting or small talk, respond with:
            {
              "type": "greeting",
              "message": "Hello! How can I assist you with shopping today?"
            }
            and stop.

            Greetings include (case-insensitive): hi, hello, hey, good morning, good afternoon, good evening, how are you, what's up.

            Step 2: Otherwise, if the user is asking about a product, respond with:
            {
              "type": "product_query",
            ...
            Rules:
            - "product_name" is null if the user is referring to the previously discussed product.
            - "requested_fields" should contain exactly what details the user wants.
            - If they want general info, include: "name", "colors", "sizes", "price".
